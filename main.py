import csv, datetime, io, itertools, os, sys, time, warnings, requests
...
ETF_LIST = [
    ...
    "PICK", "WOOD", "URA", "GDXJ", "REMX", "SIL", "XLU", "VNQ", "REET", "SRVR",
    "INDS", "TLT", "IEF", "SHY", "BND", "LQD", "HYG", "JNK", "MUB", "UUP",
    "FXE", "FXB", "FXY", "GLD", "SLV", "CORN", "WEAT", "SOYB", "DBA", "DBC",
    "GCC", "UNG", "EWJ", "EWG", "EWU", "EWA", "EWC", "FXI", "GREK"
]
ETF_LIST = sorted(list(set(ETF_LIST)))

def get_spy_holdings():
    try:
        url = "https://www.ssga.com/us/en/intermediary/etfs/library-content/products/fund-data/etfs/us/holdings-daily-us-en-spy.csv"
        res = requests.get(url, headers={"User-Agent": "Mozilla/5.0"}, timeout=15)
        lines = res.text.splitlines()
        header_idx = next(i for i, line in enumerate(lines) if "Ticker" in line or "Identifier" in line)
        df = pd.read_csv(io.StringIO("\n".join(lines[header_idx:])))
        return [s for s in df[df.columns[0]].dropna().tolist() if isinstance(s, str) and 1 <= len(s.strip()) <= 5 and s.strip().isalpha()]
...
def scrape_etf_holdings(page, etf_symbol):
    if etf_symbol == "SPY":
        ...
        if spy_symbols: return spy_symbols
    ...
    try:
        ...
        while True:
            try: page.wait_for_selector("td", timeout=10000)
            except: break
            ...
            for row in page.locator("tr").all():
                try:
                    ...
                    if cells.count() > 0:
                        ...
                        if txt and txt.isupper() and 1 <= len(txt) <= 5: all_symbols.append(txt)
                except: continue
                
            next_link = page.locator("a:has-text('Next')").filter(has_text="Next").first
            if next_link.count() > 0 and next_link.is_visible() and not next_link.evaluate("el => el.classList.contains('disabled')"):
                next_link.click(); time.sleep(2)
            else: break
    except Exception as e:
...
def analyze_holdings(etf_symbol, symbols):
    ...
    for i in range(0, len(cleaned), 15):
        ...
        try:
            data = yf.download(batch, period="max", interval="1d", group_by="ticker", progress=False, threads=True)
            ...
            for symbol in batch:
                try:
                    ...
                    if price > ema200 and ema50 > ema200 and price >= ath * 0.93 and price > ema9:
